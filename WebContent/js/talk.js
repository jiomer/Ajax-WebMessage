			...
			if (request.readyState == 4) {
				if (request.status == 200) {
					...
					if (request.responseText == 'true') { // 如果发送成功
						// 在消息容器中添加自己发送的消息
						container.innerHTML += '<li><span class="sender">你</span>对<span class="reader">'
								+ reader
								+ '</span>说:<span class="msg">'
								+ str
								+ '</span></li>';
...
function refreshContainer(list) {
	...
	for (var i = 0; i < list.length; i++) {
		...
		msgStr += '<li><span class="sender">' + m.getAttribute('s')
				+ '</span>对<span class="reader">'
				+ (m.getAttribute('r') == '' ? '所有人' : m.getAttribute('r'))
				+ '</span>说:<span class="msg">' + m.getAttribute('m')
				+ '</span></li>';
	}
	if (list.length > 0) {
...
function test() {
	...
	window.setInterval(function() {
		document.getElementById('msg').value = "TEST" + i;
		i++;
		sendMessage();
	}, 20);
}
